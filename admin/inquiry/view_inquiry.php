...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT i.*,h.product_title FROM `inquiry_list` i inner join helmet_list h on h.id = i.helmet_id where i.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="container-fluid">
    <dl>
        <dt class="text-muted">Helmet</dt>
        <dd class='pl-4 fs-4 fw-bold text-gray'><b><?= isset($product_title) ? $product_title : '' ?></b><a href="<?= base_url."?page=view_product&id=".$helmet_id ?>" target=_blank class="text-decoration-none text-gray px-2"><b><i class="fa fa-external-link-alt"></i></b></a></dd>
