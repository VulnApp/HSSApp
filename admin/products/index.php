...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">List of Helmets</h3>
			...
			<table class="table table-hover table-striped">
				...
				<tbody>
					<?php 
                        ...
                        // }
                        $brand_qry = $conn->query("SELECT id,`name` FROM `brand_list` where id in (SELECT brand_id from `helmet_list` {$bwhere}) ");
                        $brand_res = $brand_qry->fetch_all(MYSQLI_ASSOC);
                        $brand_arr = array_column($brand_res,'name','id');
						$category_qry = $conn->query("SELECT id,`name` FROM `category_list` where id in (SELECT category_id from `helmet_list` {$bwhere}) ");
                        $category_res = $category_qry->fetch_all(MYSQLI_ASSOC);
                        $category_arr = array_column($category_res,'name','id');
						$qry = $conn->query("SELECT * from `helmet_list` {$bwhere} order by `product_title` asc ");
