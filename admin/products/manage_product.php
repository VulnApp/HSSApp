<?php
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT * FROM `helmet_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
...
<div class="card card-outline card-primary rounded-0 shadow-sm my-3">
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="form-group">
                                <label for="product_title" class="control-label text-dark">Helmet Model</label>
                            ...
                            <div class="form-group">
                                <label for="category_id" class="control-label text-dark">Helmet Type</label>
                            ...
                            <div class="form-group">
                                <label for="brand_id" class="control-label text-dark">Brand</label>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" step="any" name="price" id="price" class="form-control form-control-border text-right" placeholder="Price" value ="<?php echo isset($price) ? $price : 0 ?>" required>
                            </div>
                            <div class="form-group">
                                <label for="color" class="control-label text-dark">Variants</label>
                                <textarea rows="2" name="color" id="color" class="form-control form-control-border" placeholder="color" required><?php echo isset($color) ? $color : "" ?></textarea>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="banner" class="control-label text-dark">Thumbnail/Banner</label>
                            ...
                            <div class="form-group d-flex justify-content-center">
                                <img src="<?php echo validate_image(isset($id) ? "uploads/banners/helmet-{$id}.png?v={$date_updated}" :'') ?>" alt="" id="product-cover" class="img-fluid img-thumbnail bg-dark">
                            </div>
                            <div class="form-group">
                                <label for="images[]" class="control-label text-dark">Add Images</label>
                                <div class="custom-file">
                                <input type="file" class="custom-file-input" id="customFile" name="images[]" onchange="showName($(this))" accept="image/png,image/jpeg" <?= !isset($id) ? "required" : "" ?> multiple>
                                <label class="custom-file-label truncate-1" for="customFile">Choose file</label>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="status" class="control-label text-dark">Status</label>
                                <select name="status" id="status" class="form-control form-control-border select2" data-placeholder="Select Brand Here" required>
                                    <option value="0" <?= !isset($status) && $status == 0 ? 'selected' : '' ?>>Available</option>
                                    <option value="1" <?= !isset($status) && $status == 1 ? 'selected' : '' ?>>Unavailable</option>
                                </select>
                            </div>
                        </div>
                    </div>
    ...
    <div class="card-footer rounded-0">
        ...
        <?php else: ?>
        <a class="btn btn-secondary" href="./?page=products/view_product&id=<?= $id ?>">Cancel</a>
        <?php endif; ?>
    </div>
</div>
<script>
    ...
    function showName(_this){
        var names = [] 
        if (_this[0].files && _this[0].files.length > 0) {
            Object.keys(_this[0].files).map(k=>{
                names.push(_this[0].files[k].name)
            })
            if(names.length > 0)
                _this.siblings('label').text(names.join(", "));
            else
                _this.siblings('label').text('Choose file')
        }else{
            _this.siblings('label').text('Choose file')
