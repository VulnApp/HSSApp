...
Class Master extends DBConnection {
	...
	function save_helmet(){
		...
		if(empty($id)){
			$sql = "INSERT INTO `helmet_list` set {$data} ";
		}else{
			$sql = "UPDATE `helmet_list` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Helmet Product was successfully added.";
			else
				$resp['msg'] = "Helmet Product was successfully updated.";
				if(isset($_FILES['banner']) && $_FILES['banner']['tmp_name'] != ''){
					$fname = 'uploads/banners/helmet-'.$cid.'.png';
					$dir_path =base_app. $fname;
					...
					if(!in_array($type,$allowed)){
						...
						$new_width = $width;
						if($width > $max_width){
							$perc = ($width - $max_width) / $width;
							$new_width = $max_width;
							$new_height = $height - ($height * $perc);
						}
						if($height > $max_height){
							$perc = ($height - $max_height) / $height;
				...
				if(isset($_FILES['images']) && count($_FILES['images']['tmp_name']) > 0){
					foreach($_FILES['images']['tmp_name'] as $k => $v){
						if(!empty($_FILES['images']['tmp_name'][$k])){
							if(!is_dir(base_app."uploads/helmets/"))
								mkdir(base_app."uploads/helmets/");
							if(!is_dir(base_app."uploads/helmets/{$cid}"))
								mkdir(base_app."uploads/helmets/{$cid}");
							$fname = "uploads/helmets/{$cid}/helmet-".$cid.'_'.(time()).'.png';
							$dir_path =base_app. $fname;
							$i= 1;
							while(true){
								if(!is_file($dir_path)){
									...
									$fname = "uploads/helmets/{$cid}/helmet-".$cid.'_'.(time()).'_'.$i.'.png';
									$dir_path =base_app. $fname;
									$i++;
								}
							}
	...
	function delete_helmet(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `helmet_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
	...
	function delete_img(){
		...
		if(is_file(base_app.$path)){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = 'Image Path seems not existing.';
		}
		return json_encode($resp);
	}
}
...
switch ($action) {
	...
	break;
	case 'save_product':
		echo $Master->save_helmet();
	break;
	case 'delete_product':
		echo $Master->delete_helmet();
	break;
	case 'save_inquiry':
